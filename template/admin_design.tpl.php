  ...
  function popColor(fieldName){
    ...
    for(var i=0;!found&&i<elements_count;i++){
      if(document.designform.elements[i].name==fieldName){
        ...
        window.open("main.php?include=6&session_id=<?PHP ECHO $session_id?>&formname=designform&element="+i, "colorbox", "fullscreen=no,toolbar=no,status=no,menubar=no,scrollbars=no,resizable=yes,directories=no,location=no,width="+width+",height="+height+",left="+left_pos+",top="+top_pos);
      }
    }
  }
    ...
    <TABLE class="dforeground" border="1" cellspacing="0" cellpadding="6" width="90%">
      <TR>
        <TD class="hforeground" align="center" colspan="2">
          <B><?PHP ECHO $lng["chatdesign"]?></B>
        </TD>
      </TR>
      <TR>
        <TD class="hforeground" align="left" colspan="2">
          <INPUT type="radio" name="css_source" value="0" <?PHP ECHO $css_source_0_checked?> onclick="updateFields()" onselect="updateFields()">
          Use external CSS from following URL:
          <INPUT type="text" class="textinputs" name="css_url" value="<?PHP ECHO $css_url?>" size="60" maxlength="255">
          <br /><br />
          - or -
          <br /><br />
          <INPUT type="radio" name="css_source" value="1" <?PHP ECHO $css_source_1_checked?> onclick="updateFields()" onselect="updateFields()">
...
?>
      <TR>
        <TD class="dforeground" align="left" colspan="2">
          <B><?PHP ECHO $class_data[description]?></B> (<?PHP ECHO $class_data[name]?>)
        </TD>
      </TR>
<?PHP
...
?>
      <TR>
        <TD class="hforeground" align="left">
          <?PHP ECHO $property_data[description]?> (<?PHP ECHO $property_data[name]?>)
        </TD>
<?PHP
    IF($property_data[choice]=="~text~"){
?>
        <TD class="hforeground" align="left">
          <INPUT type="text" class="textinputs" name="properties_<?PHP ECHO $class_id?>_<?PHP ECHO $property_id?>" value="<?PHP ECHO $property_data[value]?>">
        </TD>
<?PHP
...
?>
        <TD class="hforeground" align="left">
          <INPUT type="text" class="textinputs" name="properties_<?PHP ECHO $class_id?>_<?PHP ECHO $property_id?>" value="<?PHP ECHO $property_data[value]?>" maxlength="7">
          <INPUT type="button" class="buttons" value="<?PHP ECHO $lng["color"]?>" onClick="popColor('properties_<?PHP ECHO $class_id?>_<?PHP ECHO $property_id?>')">
        </TD>
<?PHP
...
?>
        <TD class="hforeground" align="left">
          <SELECT class="selects" name="properties_<?PHP ECHO $class_id?>_<?PHP ECHO $property_id?>">
<?PHP
...
?>
            <OPTION value="<?PHP ECHO $options[$i]?>" <?PHP ECHO $selected?>><?PHP ECHO $options[$i]?></OPTION>
<?PHP
      }
?>
          </SELECT>
...
?>
      <TR>
        <TD class="hforeground" align="center" colspan="2">
          <INPUT type="submit" name="submitbutton" class="buttons" value="<?PHP ECHO $lng["savechanges"]?>">
          &nbsp;
          <INPUT type="reset" name="resetbutton" class="buttons" value="<?PHP ECHO $lng["resetform"]?>">
          &nbsp;
        </TD>
      </TR>
    </TABLE>
