...
<!--
  ...
  /* This room ID */
  var this_room_id=<?PHP ECHO $session->room_id?>;

  /* This user ID */
  var this_user_id=<?PHP ECHO $session->user_id?>;
  ...
  *  Type: int
  */
  var postDelay=<?PHP ECHO $session->config->post_delay?>;
  ...
  *  Type: string
  */
  var roomlist_html="<?PHP ECHO $roomlist_html?>";

  /* Userlist frame template
  *  Type: string
  */
  var userlist_html="<?PHP ECHO $userlist_html?>";
...
IF(IS_ARRAY($sysMsg)){
  ...
  WHILE($sysMsg[$i]){
?>
  systemMessages[<?PHP ECHO $i?>]="<?PHP ECHO $sysMsg[$i]?>";
<?PHP
    $i++;
  }
}
?>
  ...
  *  Type: bool
  */
  var allowPM=<?PHP ECHO $session->config->allow_pm?>;
  ...
  */
  ...
  /**
   * The time of last refresh attempt
   * Type: int
   */
  var lastRefresh=0;
  ...
  **************************************************************************/
  function showUserList(){
    try{
      ...
      userlist.document.write("<?PHP ECHO $userlist_headers?>");
      var users_count=user.length;
      var users="";
      var muted_link="";
      var info_link="";
      ...
      for(var i=0;i<users_count;i++){
        ...
        if(user[i].Level>0){
          muted_link="<IMG src='"+imagePath+"/privileged.gif' border='0' title='<?PHP ECHO $lng["admin"]?>'>";
        }else if(user[i].ID==this_user_id){
          muted_link="<IMG src='"+imagePath+"/clearpixel.gif' border='0' title=''>";
        }else if(user[i].Muted){
          muted_link="<A href='' onclick='parent.mute("+user[i].ID+"); parent.focusInput(); return false;'><IMG src='"+imagePath+"/unmute.gif' border='0' title='<?PHP ECHO $lng["unmute"]?>'></A>";
        }else{
          muted_link="<A href='' onclick='parent.mute("+user[i].ID+"); parent.focusInput(); return false;'><IMG src='"+imagePath+"/mute.gif' border='0' title='<?PHP ECHO $lng["mute"]?>'></A>";
        }
        // 'Profile' button
        info_link="<A href='' onclick='parent.openProfileWindow("+user[i].ID+"); parent.focusInput(); return false;'><IMG src='"+imagePath+"/info.gif' border='0' title='<?PHP ECHO $lng["profile"]?>'></A>";
        // 'Whisper' button
        if(user[i].ID!=this_user_id){
          whisper_link="<A href='' onclick='parent.insertWhisper("+user[i].ID+"); parent.focusInput(); return false;'><IMG src='"+imagePath+"/whisper.gif' border='0' title='<?PHP ECHO ADDSLASHES($lng["whisper"])?>'></A>";
        }else{
          whisper_link="";
        }
        // 'Private message' button
        if(allowPM && user[i].ID!=this_user_id){
          pm_link="<A href='' onclick='parent.openPMWindow("+user[i].ID+",0); parent.focusInput(); return false;'><IMG src='"+imagePath+"/private_message.gif' border='0' title='<?PHP ECHO ADDSLASHES($lng["privatemessage"])?>'></A>";
        }else{
          pm_link="";
        }
        // Show user
  ...
  **************************************************************************/
  function prepareControlFrame(){
    try{
      ...
      control.document.write("<HTML><HEAD><?PHP ECHO STR_REPLACE("\"","'",$css)?></HEAD><BODY class='inputframe'></BODY></HTML>");
  ...
  **************************************************************************/
  function startControl(){
    ...
    if(controlRefreshID){
      try{
        lastRefresh=unixTimeStamp();
        control.document.location.href="main.php?include=<?PHP ECHO $include?>&session_id="+session_id+"&frame=c";
        clearInterval(controlRefreshID);
      }catch(e){}
    }
    try{
      controlRefreshID=setInterval("lastRefresh=unixTimeStamp(); control.document.location.href='main.php?include=<?php echo $include; ?>&session_id="+session_id+"&frame=c';", <?php echo $session->config->main_refresh*1000; ?>);
      setInterval('checkRefresher()', <?php echo round($session->config->main_refresh*1500); ?>);
    }catch(e){}
  }

  /**************************************************************************
  FUNCTION checkRefresher
  ---------------------------------------------------------------------------
  Task:
    Check refresh interval for being active and reinit it, if needed.
  ---------------------------------------------------------------------------
  Parameters: --
  ---------------------------------------------------------------------------
  Return: --
  **************************************************************************/
  function checkRefresher() {
    var nextRefresh=lastRefresh+<?php echo $session->config->main_refresh*1000; ?>;
    if (nextRefresh<unixTimeStamp()) {
      startControl();
    }
  }
  ...
  **************************************************************************/
  function firstRun(){
    try{
      ...
      inputFieldWidth=document.body.offsetWidth-<?PHP ECHO $session->config->userlist_width?>-220;
      // Initialize messages frame
      initMsgFrame();
      /* Setting up userlist and roomlist */
      <?PHP ECHO $command_line?>
      prepareControlFrame();
      showUserList();
      showRoomList();
      startControl();
  ...
  **************************************************************************/
  function initMsgFrame(){
    ...
    main.document.open();
    main.document.write("<HTML><HEAD><META http-equiv=\"Content-Language\" content=\"<?PHP ECHO $ISO_639_LNG?>\"><META http-equiv=\"Content-Type\" content=\"text/html; charset=<?PHP ECHO $lng["charset"]?>\"><?PHP ECHO STR_REPLACE("\"","'",STR_REPLACE("\r","",STR_REPLACE("\n","",$css)))?></HEAD><BODY class=\"message\" <?PHP ECHO $background?>>");
  }
  ...
  **************************************************************************/
  function insertWhisper(user_id){
    try{
      var whisper="<?PHP ECHO ADDSLASHES($lng["whisperto"])?>";
  ...
  **************************************************************************/
  function checkWhispered(){
    try{
      var whisperCmd="[<?PHP ECHO ADDSLASHES($lng["whisperto"])?>]:";
    ...
    try{
      ...
      if(i>=0&&!user[i].Muted){
        if(user[i].ID!=this_user_id||message_parts[3]<0||enableTimeStamp&&synchronize){
          ...
          if(type==1){
            // Message to you
            var whisperTemplate="<?PHP ECHO ADDSLASHES($lng["whisperedtoyou"])?>";
            var showColor=user[i].Color;
          }else if(type==2){
            // Message from you
            var whisperTemplate="<?PHP ECHO ADDSLASHES($lng["youwhisperedto"])?>";
  ...
  function insertTalkTo(user_id){
    try{
      if(user_id!=this_user_id){
        ...
        var say="<?PHP ECHO ADDSLASHES($lng["sayto"])?>";
        var arr_nr=findUserByID(user_id);
        if(arr_nr>=0){
  ...
  **************************************************************************/
  function checkSaid(){
    try{
      var sayCmd="[<?PHP ECHO ADDSLASHES($lng["sayto"])?>]:";
    ...
    try{
      ...
      if(i>=0&&!user[i].Muted){
        if(user[i].ID!=this_user_id||message_parts[3]<0||enableTimeStamp&&synchronize){
          ...
          // Prepare template
          var sayTemplate="<?PHP ECHO ADDSLASHES($lng["usersaidtouser"])?>";
