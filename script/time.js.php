  ...
  *  Type: int (0: No, 1: Yes)
  */
  var enableTimeStamp=<?PHP ECHO $session->config->timestamp_status?>;

  /* Wether to enable message post time synchronization between client and server or not
  *  Type: int (0: No, 1: Yes)
  */
  var synchronize=<?PHP ECHO $session->config->synchronize_time?>;
  ...
  **************************************************************************/
  function getHumanTime(unixTimeStamp){
    ...
    var dateFormat="<?PHP ECHO STR_REPLACE("\"","\\\"",$session->config->post_timestamp_format)?>";
  ...
  **************************************************************************/
  function showTimestampButton(){
    try{
      if(enableTimeStamp){
        input.document.timestampbutton.src="<?PHP ECHO IMAGEPATH?>/timestamp_on.gif";
      }else{
        input.document.timestampbutton.src="<?PHP ECHO IMAGEPATH?>/timestamp_off.gif";
      }
    }catch(e){}
  }
