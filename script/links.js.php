    ...
    try{
      ...
      for(var m=0;m<messagePartsLength;m++){
        if(messageParts[m].substring(0,7).toLowerCase()=="http://"){
          ...
          messageParts[m]="<A href=\"<?PHP ECHO $session->config->homepage?>/extern.php?ext="+messageParts_tmp+"\" target=\"_blank\">"+messageParts[m]+"</A>";
        }else if(messageParts[m].substring(0,4).toLowerCase()=="www."){
          // Chunk begins with 'www.'
          var messageParts_tmp=messageParts[m].split('&').join('%26');
          messageParts[m]="<A href=\"<?PHP ECHO $session->config->homepage?>/extern.php?ext=http://"+messageParts_tmp+"\" target=\"_blank\" class=\"msglink\">"+messageParts[m]+"</A>";
        }else{
