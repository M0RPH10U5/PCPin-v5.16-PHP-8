...
CLASS session EXTENDS dbaccess{
  ...
  */
  ...
  /* Flag: If not empty, then session owner was kicked.
  *  Type: int (1: Yes, 0: No)
  */
  var $kicked=0;
  ...
  **************************************************************************/
  FUNCTION cleanUp(){
    ...
    $query='SELECT id FROM '.PREFIX.'session WHERE direct_login = 0 AND last_ping + '.round($this->config->max_ping+1.5*$this->config->main_refresh).' < UNIX_TIMESTAMP()';
    $result=$this->db->query($query);
